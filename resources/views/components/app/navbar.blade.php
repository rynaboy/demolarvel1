<link rel="stylesheet" href="{{ asset('assets/css/corporate-ui-dashboard.css') }}">

<link rel="stylesheet" href="https://cdn.datatables.net/1.12.1/css/jquery.dataTables.min.css">

<style>

@import url('https://fonts.googleapis.com/css2?family=Battambang:wght@100;300;400;700;900&display=swap');
*{

        font-family: "Battambang", system-ui;
        font-weight: 400;
        font-style: normal;
}
::-webkit-scrollbar {
    display: none;
}
</style>


<nav class="navbar navbar-main navbar-expand-lg mx-5 px-0 shadow-none rounded " id="navbarBlur" >
    <div class="container-fluid py-1 px-2 ">
        <div class="row">
        <div class="col-sm-6 col-md-6 col-lg-6 col-xl-10 col-xxl-10 col-1 ">
           <nav aria-label="breadcrumb" class="gap" style="display: flex; justify-content: center; align-items: center; gap: 25px">
                <a href="{{route('dashboard')}}">                            <img  class="img {{is_current_route('members.create') ? 'img1' : ''}} {{ is_current_route('commune') ? 'img1' : '' }} " src="data:image/jpeg;base64," alt=""></a>
            <ol class="breadcrumb bg-transparent mb-1 pb-0 pt-1 px-0 me-sm-6 me-5" class="">
                <li class="breadcrumb-item text-sm text-dark active {{is_current_route('members.create') ? 'title1' : ''}} {{ is_current_route('commune') ? 'title1' : '' }}" style="font-weight: bold" aria-current="page"><?= $title ?></li>
            </ol>
            </nav>
        </div>
        </div>
        <div class="row">
            <div class="col-sm-6 col-md-6 col-lg-6 col-xl-2 col-xxl-2 ">
                <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                  <div class="mb-0 font-weight-bold breadcrumb-text text-white ">
                       <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <a href="login" onclick="event.preventDefault();
                               this.closest('form').submit();">
                               <button class="btn btn-sm  btn-white  mb-0 me-1 {{is_current_route('members.create') ? 'btn1' : ''}} {{ is_current_route('commune') ? 'btn1' : '' }}" type="submit">ចាកចេញពីប្រព័ន្ធ</button>
                             </a>
                       </form>
                       
                   </div>
                   
                   <ul class="navbar-nav " style="display: flex; justify-content:center;">
                    <li class="nav-item  d-xl-none ps-3 d-flex align-items-center ">
                    <a href="javascript:;" class="nav-link text-body p-0" id="iconNavbarSidenav">
                        <div class="sidenav-toggler-inner {{is_current_route('members.create') ? 'sidenav-toggler-inner1' : ''}} {{ is_current_route('commune') ? 'sidenav-toggler-inner1' : '' }}">
                            <i class="sidenav-toggler-line"></i>
                            <i class="sidenav-toggler-line"></i>
                            <i class="sidenav-toggler-line"></i>
                        </div>
                       </a>
                    </li>
                   </ul>
                </div>
           </div>
       </div>
    </div>
</nav>

