...
<div class="modal fade" id="create_commune" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <form action="{{ route('commune_store') }}" method="POST">
        @csrf
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">បង្កើតឃុំ/សង្កាត់ថ្មី</h1>
...
<script>
    ...
    function getCommune(searchValue = '', page = 1) {
    currentPage = page;
    const url = "{{ route('getCommune') }}";
                ...
                html += `
                    <tr style="cursor: pointer" onclick="navigateToVillage(${com.code})">
                        ...
                        <td class="align-middle text-center text-sm">
                            <span class="badge badge-sm border border-success text-success bg-success">1</span>
                        </td>
    ...
    $('#search').on('input', function() {
        getCommune($(this).val(), 1); // Reset to page 1 on search
    });

    $(document).on('click', '.pagination-link', function() {
        const page = $(this).data('page');
        getCommune($('#search').val(), page);
    });
    ...
    $(document).on('click', 'th', function() {
        ...
        getCommune($('#search').val(), currentPage);
    });
    $(document).on('click', 'th', function() {
    const column = $(this).data('column');
    sortDirection = $(this).data('direction') === 'asc' ? 'desc' : 'asc';
